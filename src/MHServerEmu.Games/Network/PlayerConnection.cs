...
namespace MHServerEmu.Games.Network
{
    ...
    public class PlayerConnection : NetClient
    {
        ...
        private bool OnUseInteractableObject(in MailboxMessage message)
        {
            ...
            if (logging)
            {
                string manualLog = $"[ModInteractNearbyAuto_MANUAL] Player [{Player}] avatar=[{avatar}] clicked [{targetName}#{useInteractableObject.IdTarget}] missionRef={(PrototypeId)useInteractableObject.MissionPrototypeRef}";
                Logger.Trace(manualLog);
                ModInteractNearbyAutoLogCollator.WriteLine(Player.Id, manualLog);
            }
